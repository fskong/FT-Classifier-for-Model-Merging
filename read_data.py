...
def get_train_val_data(data_path, n_train_per_class=2000,
                       ...
                       model="../checkpoints/bert-base-uncased", offset=0, seed=0):
    tokenizer = BertTokenizer.from_pretrained('../checkpoints/bert-base-uncased', local_files_only=True)
...
def get_test_data(data_path, max_seq_len=256,
                  model='../checkpoints/bert-base-uncased', offset=0):
    tokenizer = BertTokenizer.from_pretrained('../checkpoints/bert-base-uncased', local_files_only=True)
    ...
    for i in range(task_num):
        ...
        train_dataset, val_dataset = \
            get_train_val_data(data_path, train_class_size,
                               val_class_size, offset=offsets[i], seed=seed)
        test_dataset = get_test_data(data_path + '/test.csv', offset=offsets[i])
...
def prepare_dataloaders_imdb(data_dir, train_batch_size):
    data_path = os.path.join(data_dir)
    train_dataset = get_test_data(data_path)
    train_loader = DataLoader(train_dataset, batch_size=train_batch_size,
                              shuffle=True, drop_last=True)
    return train_loader
